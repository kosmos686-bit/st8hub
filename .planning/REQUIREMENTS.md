...
- [x] **ROUTE-02**: Запросы короче 100 токенов без признаков сложности маршрутизируются на Haiku
- [x] **ROUTE-03**: Запросы с кодом, анализом документов или длиной > 500 токенов маршрутизируются на Sonnet
- [x] **ROUTE-04**: Правила маршрутизации (пороги, ключевые слова) настраиваются в `.env` без правки кода
...
- [x] **METRIC-02**: Команда `/stats` в Telegram возвращает агрегированную сводку за текущий день (кол-во вызовов, токены, сумма расходов по моделям)

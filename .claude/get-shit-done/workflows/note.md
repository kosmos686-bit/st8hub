...
- **Global scope**: `C:/st8-workspace/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/st8-workspace/.claude/notes/`)
...
2. Glob `C:/st8-workspace/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/st8-workspace/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/st8-workspace/.claude/notes/` — works in any directory

...
- `нет X` / `замени X` → Codex Haiku подбирает замену с сохранением КБЖУ
...
- `ANTHROPIC_API_KEY` — Codex API (Haiku для дешёвых задач, Sonnet для сложных)
...
- AGENTS.md переписан с полным контекстом проекта
